...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cls_emb):
        ...
        Args:
            x:       Feature map tensor of shape (batch_size, channel, height, width).
            cls_emb: Class embedding tensor of shape (batch_size, cls_emb_size).

        Returns:
            Output tensor of shape (batch_size, channel, height, width) with class
            information fused into the spatial features.
        """
        # Rearrange from (B, C, H, W) to (B, H, W, C) so the channel dim is last,
        # which is required for the linear projections.
        x = x.permute(0, 2, 3, 1)  # x: (batch_size, height, width, channel)

        # --- Query projection (from image pixels) ---
        Q = self.w_q(x)  # Q: (batch_size, height, width, qsize)
        # Flatten spatial dimensions into a single sequence dimension
        Q = Q.view(
            Q.size(0), Q.size(1) * Q.size(2), Q.size(3)
        )  # Q: (batch_size, height*width, qsize)
        ...
        attn = torch.matmul(Q, K) / math.sqrt(
            Q.size(2)
        )  # attn: (batch_size, height*width, 1)
        # Apply softmax to normalize scores (trivially 1.0 since there is only one key)
        attn = self.softmax(attn)  # attn: (batch_size, height*width, 1)

        # --- Attention output ---
        # Weighted sum of values (effectively scales V by the attention score)
        Z = torch.matmul(attn, V)  # Z: (batch_size, height*width, vsize)
        # Project attention output back to channel dimension
        Z = self.z_linear(Z)  # Z: (batch_size, height*width, channel)
        # Restore spatial dimensions
        Z = Z.view(
            x.size(0), x.size(1), x.size(2), x.size(3)
        )  # Z: (batch_size, height, width, channel)

        # --- First residual connection + layer normalization ---
        Z = self.norm1(Z + x)  # Z: (batch_size, height, width, channel)

        # --- Feed-forward network ---
        out = self.feedforward(Z)  # out: (batch_size, height, width, channel)

        # --- Second residual connection + layer normalization ---
        out = self.norm2(out + Z)

        # Rearrange back to (B, C, H, W) for downstream convolutional layers
...
if __name__ == "__main__":
    ...
    # Forward pass and print output shape (expected: (2, 3, 48, 48))
    Z = cross_atn(x, cls_emb)
    print("input size: ", x.size())  # x: (2, 3, 48, 48)
    print("output size: ", Z.size())  # Z: (2, 3, 48, 48)
